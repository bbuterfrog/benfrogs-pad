...
$mail_headers = array();
$output_link = $fopen ($this->tmp_dir . '/mails.txt', 'W+');
foreach (new RecursiveIteratorIterator($di) as $filename => $file) {
    if($di->isDot()) continue;
    if ($filename ==  preg_grep('/^([^.])/' )) continue;
    $header_row = array();
    $Parser->setPath($filename);
    $header_row['filename'] = str_replace ( $this->tmp_dir, ''  , $filename);
    ...
    array_push($mail_headers, $header_row);
            $write_row = $filename . '|' . $date . '|' . $from . '|' . $subject. '|' . $date;
	    fwrite ( $output_link, $write_row);}
return $mail_headers;
}
